...
import {getByWalletId} from '../../api/charizoid/coin'
...
const MiddleInfo = () => {
  ...
  useEffect(() => {
    ...
    const curAmount = getCookie('charizoidCurrentAmount')
    const curValue = getCookie('charizoidCurrentValue')
    if (curAmount != '')
      setCurrentAmount(parseInt(curAmount))
    if (curValue != '')
      setCurrentValue(parseFloat(curValue))
    const tmpAmounts = getCookie('charizoidAmounts')
    const tmpEarnings = getCookie('charizoidEarnings')
    const tmpSigns = getCookie('charizoidSigns')
    if (tmpAmounts != '') {
  ...
  useEffect(() => {
    if (status === 'idle') {
      ...
      if (setting.walletId != null && setting.walletId != '' && data != null && data.length != 0 && data[0] != -1) {
        ...
        setCookie('charizoidCurrentAmount', data[0], 10)
        setCookie('charizoidCurrentValue', data[0] * setting?.price, 10)
        //--set Wallet values
        setCookie('charizoidAmounts', JSON.stringify(tmpAmounts), 10)
        setCookie('charizoidEarnings', JSON.stringify(tmpEarnings), 10)
        setCookie('charizoidSigns', JSON.stringify(tmpSigns), 10)
