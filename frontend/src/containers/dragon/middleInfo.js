...
import {getByWalletId} from '../../api/dragon/coin'
...
const MiddleInfo = () => {
  ...
  useEffect(() => {
    ...
    const curAmount = getCookie('dragonCurrentAmount')
    const curValue = getCookie('dragonCurrentValue')
    if (curAmount != '')
      setCurrentAmount(parseInt(curAmount))
    if (curValue != '')
      setCurrentValue(parseFloat(curValue))
    const tmpAmounts = getCookie('dragonAmounts')
    const tmpEarnings = getCookie('dragonEarnings')
    const tmpSigns = getCookie('dragonSigns')
    if (tmpAmounts != '') {
  ...
  useEffect(() => {
    if (status === 'idle') {
      ...
      if (setting.walletId != null && setting.walletId != '' && data != null && data.length != 0 && data[0] != -1) {
        ...
        setCookie('dragonCurrentAmount', data[0], 10)
        setCookie('dragonCurrentValue', data[0] * setting?.price, 10)
        //--set Wallet values
        setCookie('dragonAmounts', JSON.stringify(tmpAmounts), 10)
        setCookie('dragonEarnings', JSON.stringify(tmpEarnings), 10)
        setCookie('dragonSigns', JSON.stringify(tmpSigns), 10)
