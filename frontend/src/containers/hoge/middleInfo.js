...
const MiddleInfo = () => {
  ...
  useEffect(() => {
    ...
    const curAmount = getCookie('hogeCurrentAmount')
    const curValue = getCookie('hogeCurrentValue')
    if (curAmount != '')
      setCurrentAmount(parseInt(curAmount))
    if (curValue != '')
      setCurrentValue(parseFloat(curValue))
    const tmpAmounts = getCookie('hogeAmounts')
    const tmpEarnings = getCookie('hogEarnings')
    const tmpSigns = getCookie('hogeSigns')
    if (tmpAmounts != '') {
  ...
  useEffect(() => {
    if (status === 'idle') {
      ...
      if (setting.walletId != null && setting.walletId != '' && data != null && data.length != 0 && data[0] != -1) {
        ...
        setCookie('hogeCurrentAmount', data[0], 10)
        setCookie('hogeCurrentValue', data[0] * setting?.price, 10)
        //--set Wallet values
        setCookie('hogeAmounts', JSON.stringify(tmpAmounts), 10)
        setCookie('hogeEarnings', JSON.stringify(tmpEarnings), 10)
        setCookie('hogeSigns', JSON.stringify(tmpSigns), 10)
